...
@php
    $initialTab = 'profile';
    if (request()->has('tab')) {
        $initialTab = in_array(request()->get('tab'), ['profile','activity','settings']) ? request()->get('tab') : 'profile';
    }
@endphp
...
<div class="row g-0" x-data="{ activeTab: '{{ $initialTab }}' }">
        ...
        <div class="d-flex flex-column h-100 px-3 pt-4 p-5" style="background:#04278c; min-height:100vh; border-radius:0 20px 20px 0;">
            ...
            <nav class="flex-grow-1">
                <div class="rounded-4 p-3 mb-3" style="background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15);">
                    ...
                    <a href="#" @click.prevent="activeTab='profile'; window.history.replaceState({},'','?tab=profile')"
                       ...
                       :style="activeTab==='profile' ? 'background:rgba(255,255,255,0.15)' : ''">
                        ...
                        <span class="text-white fw-semibold" style="font-family:'Mona Sans',sans-serif; font-size:16px;"
                              :style="activeTab==='profile' ? '' : 'opacity:0.7'">{{ __('ui.profile') }}</span>
                    </a>

                    <a href="#" @click.prevent="activeTab='activity'; window.history.replaceState({},'','?tab=activity')"
                       class="d-flex align-items-center gap-3 text-decoration-none mb-2 py-1 rounded-3 px-2"
                    ...
                    <a href="#" @click.prevent="activeTab='settings'; window.history.replaceState({},'','?tab=settings')"
            ...
            <div class="mt-auto pb-4 ps-4">
                <a href="{{ route('about.index') }}"
                   ...
                   style="background:rgba(255,255,255,0.08);">
                    ...
                    <span class="fw-semibold" style="font-family:'Mona Sans',sans-serif; font-size:16px;">{{ __('ui.about_us') }}</span>
                </a>
                <form id="sidebar-logout" action="{{ route('auth.logout') }}" method="POST">
                    ...
                    <a href="#" onclick="document.getElementById('sidebar-logout').submit();"
                       ...
                       style="opacity:0.6;">
                        ...
                        <span class="fw-semibold" style="font-family:'Mona Sans',sans-serif; font-size:16px;">{{ __('ui.logout') }}</span>
                    </a>
                </form>
            </div>
        </div>
    ...
    {{-- ============ MAIN CONTENT ============ --}}
    <div class="col" style="background:#f8f9fa;">

        {{-- Top Profile Banner (shared) --}}
        <div class="position-relative overflow-hidden rounded-4 mt-3 mx-3 mb-4"
